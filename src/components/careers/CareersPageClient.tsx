...
const DynamicJobExplorerClient = dynamic(
  () =>
    import("@/components/careers/JobExplorer").then(
      (mod) => mod.JobExplorerClient,
    ),
  {
    ssr: false,
...
const t = {
    ...
    values: [
      {
        ...
        description:
          "Bekerja pada proyek-proyek lingkungan nyata di seluruh Indonesia.",
      },
      {
      ...
      },
      {
        ...
        description:
          "Bergabunglah dengan tim ahli yang solid dan saling mendukung.",
      },
      {
    ...
    steps: [
      {
        title: "Daftar Online",
        description:
          "Lengkapi profil dan kirimkan lamaran Anda melalui portal karir kami.",
      },
      {
        title: "Psikotes",
        description:
          "Kerjakan tes psikologi untuk mengukur potensi dan kesesuaian Anda.",
      },
      {
        title: "Seleksi Administrasi",
        description:
          "Tim rekrutmen akan meninjau kelengkapan profil dan hasil psikotes Anda.",
      },
      {
        title: "Wawancara",
        description:
          "Bertemu dengan HR dan calon user untuk diskusi lebih mendalam.",
      },
      {
        title: "Tawaran Kerja",
        description:
          "Kandidat terpilih akan menerima tawaran kerja resmi dari kami.",
      },
    ],
  },
  OfficeSpotlight: {
    ...
    subtitle:
      "Berpusat di Yogyakarta, kantor Environesia menjadi ruang kolaborasi, koordinasi, dan pengembangan inovasi untuk menghadirkan solusi lingkungan yang berdampak.",
  },
  HowToApply: {
    ...
    steps: [
      ...
      },
      {
        title: "Cari Lowongan",
        description:
          "Jelajahi berbagai posisi yang tersedia dan temukan yang cocok.",
      },
      {
        title: "Kirim Lamaran",
        description:
          "Unggah CV terbaru Anda dan kirimkan lamaran dengan mudah.",
      },
      {
        title: "Pantau Proses",
  ...
  FAQ: {
    ...
    subtitle:
      "Jawaban atas pertanyaan umum seputar proses lamaran kerja di Environesia.",
...
const HeroSection = ({
  ...
  sectionData: LandingSection | null;
}) => {
  const title = sectionData?.title || t.Hero.title;
  const subtitle = sectionData?.description || t.Hero.subtitle;
  const backgroundImageUrl = "/images/LP-1.jpg";
  const primaryButtonText = sectionData?.primaryButtonText || t.Hero.ctaPrimary;
  const secondaryButtonText =
    sectionData?.secondaryButtonText || t.Hero.ctaSecondary;
  ...
  return (
    <section id="hero" className="relative w-full bg-background">
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex min-h-[70vh] flex-col items-center justify-center pb-40 pt-32 text-center lg:min-h-dvh">
          ...
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4 w-full max-w-xs sm:max-w-none">
            <Button
              ...
              asChild
            >
              <a href="#lowongan">{primaryButtonText}</a>
            </Button>
            <Button
              size="lg"
              variant="secondary"
              ...
              asChild
            >
              <Link href="/careers/login">{secondaryButtonText}</Link>
            </Button>
          </div>
        </div>
      </div>
...
const JobExplorerSection = () => {
  ...
  return (
    <section
      id="lowongan"
      className="w-full scroll-mt-20 pt-36 pb-16 lg:pt-24 lg:pb-24"
    >
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
...
const EcosystemSection = () => {
  ...
  const { data: companiesFromHook, isLoading } =
    useCollection<EcosystemCompany>(ecosystemQuery);

  const companies = React.useMemo(() => {
    if (!companiesFromHook) return [];
    return [...companiesFromHook].sort(
  ...
  return (
    <section
      id="ekosistem"
      className="w-full relative py-24 lg:py-40 overflow-hidden bg-background scroll-mt-20"
    >
      ...
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {isLoading && (
          <div className="grid grid-cols-1 gap-12 sm:grid-cols-2 lg:grid-cols-3">
            {[...Array(6)].map((_, i) => (
              <Card key={i} className="rounded-[3.5rem] h-[300px]">
            ...
            {companies.map((company) => (
              ...
              >
                <div className="relative flex flex-col h-full bg-background rounded-[3.2rem] overflow-hidden">
                  {/* Logo Container - Large and Clean */}
                  <div className="flex h-40 w-full items-center justify-center px-4">
                    <img
                      ...
                      className="block h-auto max-h-32 min-h-[90px] w-auto min-w-[220px] max-w-[360px] object-contain opacity-100"
                      onError={(e) => {
                        e.currentTarget.src = getLocalCompanyLogo(
                          company.name || (company as any).companyName,
                        );
                      }}
                    />
                  </div>
                ...
                <div
                  className={cn(
                    ...
                    index % 2 === 0
                      ...
                      : "md:text-right md:pr-[calc(50%+2.5rem)]",
                  )}
                >
...
const OfficeSpotlightSection = ({
  ...
  const bgImage = sectionData?.backgroundImageUrl
    ? `url(${sectionData.backgroundImageUrl})`
    : "url('/images/Banner%20Kantor.jpg')";
        ...
        <div className="mt-16 text-center">
          <Button size="lg" asChild>
            <Link href="/careers/register">
              {t.HowToApply.cta} <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
          </Button>
        </div>
      </div>
        ...
        <Accordion type="single" collapsible className="mt-12 w-full space-y-4">
          {questions.map((item, i) => (
            <AccordionItem
              key={i}
              value={`item-${i}`}
              className="rounded-xl border bg-background px-6 shadow-sm"
            >
              <AccordionTrigger className="py-5 text-lg text-left">
                {item.q}
              </AccordionTrigger>
        ...
        <div className="py-8 grid grid-cols-1 md:grid-cols-3 gap-8">
          <div>
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-lg font-bold">Vacancies</span>
            </div>
...
export function CareersPageClient() {
  ...
  const sectionsQuery = useMemoFirebase(
    () =>
      query(
        collection(firestore, "landing_sections"),
        where("isActive", "==", true),
        orderBy("order"),
      ),
    [firestore],
  );
  const { data: sections } = useCollection<LandingSection>(sectionsQuery);

  const heroSectionData = useMemo(
    () => sections?.find((s) => s.sectionKey === "hero") || null,
    [sections],
  );
  const basecampSectionData = useMemo(
    () => sections?.find((s) => s.sectionKey === "basecamp") || null,
    [sections],
  );
