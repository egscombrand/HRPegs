...
>(({
  ...
  const minYear = minYearProp || (mode === 'dob' ? 1950 : new Date().getFullYear() - 100);
  const maxYear = maxYearProp || (mode === 'dob' ? new Date().getFullYear() : new Date().getFullYear() + 5);
          ...
          <div key={day.toISOString()} className="p-0.5">
            <Button
              ...
              className={cn(
                ...
                value && isSameDay(day, value) && 'bg-primary text-primary-foreground hover:bg-primary hover:text-primary-foreground',
              )}
            >
              {format(day, 'd')}
            </Button>
  ...
  const renderMonthView = () => (
    <>
        <div className="p-2 text-center text-sm font-medium">{format(cursorDate, 'yyyy')}</div>
