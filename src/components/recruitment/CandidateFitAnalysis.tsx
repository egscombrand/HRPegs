...
const decisionConfig = {
  advance_interview: {
    label: "Lanjutkan ke Wawancara",
    icon: CheckCircle,
    className: "text-green-600",
  },
  advance_test: {
    label: "Lanjutkan ke Tes",
    icon: BrainCircuit,
...
export function CandidateFitAnalysis({
  profile,
  job,
  application,
  compact,
}: CandidateFitAnalysisProps) {
  const [isLoading, setIsLoading] = useState(false);
  const [analysis, setAnalysis] = useState<CandidateFitAnalysisOutput | null>(
    null,
  );
  ...
  const Decision = analysis
    ? decisionConfig[analysis.recommendedDecision]
    : null;

  const chartData = analysis
    ? Object.entries(analysis.scoreBreakdown).map(([key, value]) => {
        if (key === "cultureFit") {
          const cultureFitValue =
            typeof value === "object" ? value.score : value;
          return {
            name: scoreLabels["cultureFitScore"],
            score: cultureFitValue,
          };
        }
        return {
          name: scoreLabels[key as keyof typeof scoreLabels] || key,
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600 max-w-xs mx-auto">
              Klik tombol di bawah untuk memulai analisis AI pada CV kandidat
              ini.
            </p>
          </div>
          <Button onClick={handleAnalyze}>Mulai Analisis Sekarang</Button>
        </div>
      )}
        ...
        <div className="space-y-6">
          {analysis.confidence.level === "low" && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                {analysis.confidence.reasons.join(" ")} Analisis mungkin tidak
                akurat.
              </AlertDescription>
            </Alert>
          )}
                ...
                <ChartContainer config={chartConfig} className="h-64 w-full">
                  ...
                  >
                    ...
                    <Bar
                      ...
                      radius={6}
                      background={{ fill: "hsl(var(--secondary))", radius: 6 }}
                    >
                    ...
                    return (
                      <TableRow key={i}>
                        ...
                        <TableCell className="text-center">
                          ...
                          >
                            {item.type === "must-have" ? "WAJIB" : "SEBAIKNYA"}
                          </Badge>
                        </TableCell>
                        <TableCell>
                          <div
                            className={cn(
                              "flex items-center justify-center gap-1.5 font-black text-[10px] uppercase",
                              match.className,
                            )}
                          >
            ...
            >
              <ul className="space-y-4">
                {analysis.strengths.map((item, i) => (
                  <li
                    key={i}
                    className="bg-green-500/10 p-5 rounded-2xl border border-green-500/20"
                  >
                    <p className="font-black text-base text-green-500">
                      {item.strength}
                    </p>
                    <p className="text-sm text-green-400/80 italic mt-2 leading-relaxed">
                      &ldquo;{item.evidence_from_cv}&rdquo;
                    </p>
                  </li>
                ))}
            ...
            >
              <ul className="space-y-4">
                {analysis.gapsRisks.map((item, i) => (
                  ...
                  >
                    <p className="font-black text-base text-red-500">
                      {item.gap}
                    </p>
                    <div className="mt-3 space-y-2">
                      <p className="text-sm text-red-400/90 font-medium leading-relaxed underline decoration-red-500/30 underline-offset-4">
                        Impact: {item.impact}
                      </p>
                      <p className="text-xs text-red-50/70 font-medium leading-relaxed bg-red-500/20 p-3 rounded-xl">
                        <strong className="text-red-400 uppercase tracking-tighter">
                          Mitigation:
                        </strong>{" "}
                        {item.onboarding_mitigation}
                      </p>
                    </div>
                  </li>
                ))}
            ...
            <div className="p-6 rounded-3xl bg-red-500/10 border-2 border-dashed border-red-500/30">
              ...
              <ul className="list-disc list-inside space-y-2">
                {analysis.redFlags.map((flag, i) => (
                  <li key={i} className="text-sm font-bold text-red-800">
                    {flag}
                  </li>
                ))}
              </ul>
          ...
          <AnalysisSection
            title="Interview Questions Strategy"
            ...
            >
              <ul className="space-y-2">
                {analysis.quickTestRecommendation.map((item, i) => (
                  <li
                    key={i}
                    className="flex gap-2 items-start text-xs font-bold leading-relaxed"
                  >
          ...
          {application.cvTextExtractedAt && (
            <div className="text-center text-[10px] font-bold text-slate-600/70 pt-8 border-t border-dashed">
              <p>
                ...
                {format(
                  application.cvTextExtractedAt.toDate(),
                  "dd MMM yyyy, HH:mm",
                )}{" "}
                WIB. Total {application.cvCharCount} karakter CV diproses.
              </p>
            </div>
          )}
        </div>
