...
const AddressView = ({
  ...
  title: string;
  address?: Partial<Profile["addressKtp"]> | string;
}) => {
  if (!address) return <InfoRow label={title} value="-" />;
  if (typeof address === "string")
    return <InfoRow label={title} value={address} />;
...
export function ProfileView({ profile }: { profile: Profile }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <SectionTitle icon={<Info className="h-5 w-5" />}>
            Tentang Saya & Ekspektasi
          </SectionTitle>
        </CardHeader>
      ...
      {profile.organizationalExperience &&
        profile.organizationalExperience.length > 0 && (
          <Card>
            <CardHeader>
              <SectionTitle icon={<Building className="h-5 w-5" />}>
