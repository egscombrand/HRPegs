            ...
            {[1, 2, 3, 4, 5].map((s) => (
                <button
                    ...
                    className={cn(
                        ...
                        s <= value ? "text-teal-400 scale-110" : "text-slate-700 hover:text-teal-400/50"
                    )}
                >
...
export function PostInterviewEvaluationSection({ application, job, internalUsers }: PostInterviewEvaluationSectionProps) {
  ...
  // Form State
  const [recommendation, setRecommendation] = useState<PostInterviewReviewScore | ''>('');
  ...
  const reviewsRef = useMemo(() => 
    application.id ? collection(firestore, 'applications', application.id, 'post_interview_reviews') : null
  ...
  const handleSubmit = async () => {
    ...
    if (communicationScore === 0 || attitudeScore === 0 || fitScore === 0) {
        toast({ 
            ...
            description: 'Mohon berikan nilai untuk aspek komunikasi, attitude, dan kesesuaian role.' 
          });
          return;
    }
    ...
    try {
      const reviewRef = doc(firestore, 'applications', application.id, 'post_interview_reviews', userProfile.uid);
      ...
      // Update Summary
      try {
        const reviewsSnap = await getDocs(collection(firestore, 'applications', application.id!, 'post_interview_reviews'));
        const allReviews = reviewsSnap.docs.map(d => d.data() as PostInterviewReview);
        ...
        const summary: PostInterviewEvaluationSummary = {
          ...
          progress: assignedUids.length > 0 ? (allReviews.length / assignedUids.length) * 100 : 100,
        ...
        console.warn("Post-interview review saved, but summary sync failed:", summaryError);
      }

      toast({ title: 'Berhasil', description: 'Penilaian pasca-wawancara telah disimpan.' });
    } catch (error: any) {
      toast({ variant: 'destructive', title: 'Gagal Menyimpan', description: error.message || 'Error saving post-interview review.' });
    } finally {
      ...
      <CardHeader className="bg-teal-500/[0.03] pb-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="flex items-center gap-6">
                ...
                <div>
                    <CardTitle className="text-3xl font-black tracking-tighter uppercase text-slate-100">Evaluasi Pasca-Wawancara</CardTitle>
                    <CardDescription className="text-teal-400 font-bold italic">Penilaian berdasarkan hasil interview langsung</CardDescription>
                </div>
            </div>
            ...
            <div className="space-y-12 animate-in fade-in slide-in-from-top-6 duration-1000">
                <div className="flex flex-col lg:flex-row items-center justify-between gap-8 p-10 rounded-[2.5rem] bg-slate-950/40 border border-slate-800 shadow-2xl ring-1 ring-white/5">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <h3 className="text-2xl font-black uppercase tracking-tighter text-slate-100 italic">Progress Penilaian Tim (Pasca)</h3>
                            <div className="space-y-1">
                                <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">
                                    <span className="text-teal-400">{reviews?.length || 0}</span> dari <span className="text-slate-300">{potentialReviewers.length}</span> evaluator sudah submit
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap justify-center gap-4">
                        <div className="flex flex-col items-center gap-1.5 min-w-[100px]">
                            <Badge className="bg-teal-500/90 hover:bg-teal-500 text-white px-6 py-2 rounded-2xl font-black text-xs shadow-lg shadow-teal-500/20 w-full justify-center">{reviews?.length || 0} SELESAI</Badge>
                            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Submit</span>
                        </div>
                        <div className="flex flex-col items-center gap-1.5 min-w-[100px]">
                            <Badge className="bg-amber-500/90 hover:bg-amber-500 text-white px-6 py-2 rounded-2xl font-black text-xs shadow-lg shadow-amber-500/20 w-full justify-center">{pendingReviewers.length} BELUM</Badge>
                            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Submit</span>
                        </div>
                        <div className="flex flex-col items-center gap-1.5 min-w-[100px]">
                            ...
                            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Evaluators</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        {canReview && (
            <div className="space-y-12 p-12 rounded-[3.5rem] bg-slate-950/40 border border-slate-800 shadow-2xl ring-1 ring-white/5">
                 <div className="flex items-center gap-5">
                    ...
                    <div>
                        <h3 className="text-2xl font-black tracking-tight text-slate-100 uppercase italic">Form Penilaian Saya (Pasca)</h3>
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-1">Berikan feedback mendalam setelah berinteraksi langsung.</p>
                    </div>
                 </div>

                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div className="lg:col-span-2 space-y-6">
                        <label className="text-xs font-black text-slate-500 uppercase tracking-[0.25em] pl-4 border-l-4 border-teal-500">REKOMENDASI AKHIR</label>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {(Object.keys(postScoreLabels) as PostInterviewReviewScore[]).map((s) => (
                                <button
                                    key={s}
                                    type="button"
                                ...
                                >
                                    <span className="uppercase tracking-tight leading-tight relative z-10">{postScoreLabels[s]}</span>
                                    {recommendation === s ? <UserCheck className="h-6 w-6 relative z-10" /> : <Plus className="h-5 w-5 opacity-0 group-hover:opacity-100 transition-all group-hover:translate-x-1" />}
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-8 p-10 rounded-[2.5rem] bg-teal-500/[0.03] border border-teal-500/10 shadow-inner">
                        <StarRating label="Komunikasi" value={communicationScore} onChange={setCommunicationScore} />
                        <StarRating label="Attitude / Perilaku" value={attitudeScore} onChange={setAttitudeScore} />
                        <StarRating label="Kesesuaian dengan Role" value={fitScore} onChange={setFitScore} />
                    </div>

                    <div className="md:col-span-2 space-y-5">
                        <label className="text-xs font-black text-slate-500 uppercase tracking-[0.25em] pl-4 border-l-4 border-teal-500">CATATAN UMUM (WAJIB)</label>
                ...
                <div className="flex items-center justify-between mt-8 p-4 rounded-3xl bg-teal-900/10 border border-teal-500/10">
                    <div className="flex items-center gap-3 text-sm text-teal-300">
                        ...
                        <span className="font-semibold italic">Penilaian interview bersifat final dan rahasia.</span>
                    </div>
                    <Button 
        ...
        <div className="space-y-12">
            <div className="flex items-center justify-between px-6">
                <div className="flex items-center gap-5">
                    ...
                    <h3 className="text-2xl font-black uppercase tracking-tighter text-slate-100 italic">Tim Assessment (Pasca)</h3>
                </div>
                <Button variant="outline" size="lg" onClick={() => setShowAllReviews(!showAllReviews)} className="rounded-[1.5rem] font-black uppercase border-2 border-slate-800 text-slate-400 hover:bg-teal-600 hover:text-white hover:border-teal-600 transition-all text-[10px] tracking-widest px-8">
                ...
                <div className="space-y-10">
                    {reviews && reviews.length > 0 ? (
                        [...reviews].sort((a,b) => b.updatedAt.toMillis() - a.updatedAt.toMillis()).map((review) => (
                            <div key={review.reviewerUid} className="p-12 rounded-[4rem] bg-slate-950/20 border border-slate-800 shadow-2xl space-y-10 group transition-all duration-700 hover:bg-slate-950/40 hover:border-teal-500/20 relative overflow-hidden ring-1 ring-white/5">
                                <div className="flex flex-col sm:flex-row items-start justify-between gap-8 relative z-10">
                                    <div className="flex items-center gap-6">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-4 text-[10px] font-black text-slate-500 uppercase tracking-widest mt-2">
                                                <Badge variant="outline" className="border-teal-500/30 text-teal-400 font-black uppercase bg-teal-500/5 px-4 py-1 rounded-full">{review.reviewerRole}</Badge>
                                    ...
                                    <Badge className={cn("px-10 py-4 rounded-full text-[10px] font-black uppercase shadow-2xl scale-110 sm:scale-100", postScoreColors[review.recommendation])}>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-8 rounded-[2.5rem] bg-slate-950/40 border border-slate-800 shadow-inner">
                                    <div className="space-y-1">
                                        <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Komunikasi</p>
                                        <div className="flex text-teal-400 pb-1">
                                            {[1,2,3,4,5].map(i => <Star key={i} className={cn("h-4 w-4 fill-current", i > review.communicationScore && "text-slate-800")} />)}
                                        </div>
                                    </div>
                                    <div className="space-y-1">
                                        <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Attitude</p>
                                        <div className="flex text-teal-400 pb-1">
                                            {[1,2,3,4,5].map(i => <Star key={i} className={cn("h-4 w-4 fill-current", i > review.attitudeScore && "text-slate-800")} />)}
                                        </div>
                                    </div>
                                    <div className="space-y-1">
                                        <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Role Fit</p>
                                        <div className="flex text-teal-400 pb-1">
                                            {[1,2,3,4,5].map(i => <Star key={i} className={cn("h-4 w-4 fill-current", i > review.fitScore && "text-slate-800")} />)}
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="text-center py-40 bg-slate-900/20 rounded-[5rem] border-4 border-dashed border-slate-800 relative overflow-hidden">
                             <Activity className="h-28 w-28 mx-auto text-slate-800 mb-8 opacity-20" />
                             <p className="text-slate-600 text-2xl font-black uppercase tracking-[0.3em] opacity-40">Belum Ada Penilaian Pasca-Wawancara</p>
                        </div>
                    )}
                </div>
            )}
