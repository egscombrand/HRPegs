...
import type { Profile, Education, WorkExperience, OrganizationalExperience, Certification, JobApplication, Job } from "@/lib/types";
...
export function CandidateStepContent({ profile, application, activeStep, job }: { profile: Profile, application: JobApplication, activeStep: number, job: Job }) {
  const birthDateValue = React.useMemo(() => {
    ...
    return isNaN(date.getTime()) ? null : format(date, 'dd MMMM yyyy', { locale: idLocale });
  }, [profile.birthDate]);
      ...
      return (
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-10">
            ...
            <InfoBlock label="Email" value={application.candidateEmail} icon={Mail} />
            <InfoBlock label="Telepon" value={profile.phone} icon={Phone} />
            <InfoBlock label="Jenis Kelamin" value={profile.gender} />
            <InfoBlock label="Tempat Lahir" value={profile.birthPlace} icon={MapPin} />
            <InfoBlock label="Tanggal Lahir" value={birthDateValue} icon={Calendar} />
            <InfoBlock label="No e-KTP (NIK)" value={profile.eKtpNumber} />
            <InfoBlock label="NPWP" value={profile.hasNpwp ? profile.npwpNumber : 'Tidak Ada'} />
            <InfoBlock label="Status WFO" value={profile.willingToWfo ? 'Bersedia (WFO)' : 'Tidak Bersedia'} />
          </div>
          ...
          <div className="space-y-6">
            {profile.education?.length ? (
              profile.education.map((edu, idx) => (
                <div key={idx} className="group flex gap-6 p-8 rounded-3xl border bg-card hover:border-primary/50 transition-all duration-300 shadow-sm hover:shadow-xl">
                  ...
                  <div className="flex-1 space-y-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                       <Badge variant="secondary" className="px-4 py-1.5 text-sm font-bold bg-primary/10 text-primary hover:bg-primary/20 border-transparent">IPK: {edu.gpa || '-'}</Badge>
                       <Badge variant="outline" className="px-4 py-1.5 text-sm font-bold border-2">{edu.isCurrent ? 'Masih Menempuh' : 'Tamat / Lulus'}</Badge>
                    </div>
                    {edu.thesisTitle && (
                      <div className="mt-4 p-5 rounded-2xl bg-muted/30 border-2 border-dashed border-muted-foreground/20">
                        <p className="text-[10px] font-black uppercase text-muted-foreground mb-1">Judul TA / Skripsi</p>
                        <p className="text-base font-bold italic">{edu.thesisTitle}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <EmptyState icon={GraduationCap} text="Belum ada data pendidikan" />
            )}
          </div>
        </div>
      );
          ...
          <div className="space-y-6">
            {profile.workExperience?.length ? (
              profile.workExperience.map((exp, idx) => (
                <div key={idx} className="group flex gap-6 p-8 rounded-3xl border bg-card hover:border-blue-500/50 transition-all duration-300 shadow-sm hover:shadow-xl">
                  ...
                  <div className="flex-1 space-y-4">
                    ...
                    {!exp.isCurrent && exp.reasonForLeaving && (
                       <div className="flex items-start gap-2 text-muted-foreground bg-muted/10 p-4 rounded-xl">
                          ...
                          <p className="text-sm font-medium italic">Alasan Berhenti: {exp.reasonForLeaving}</p>
                       </div>
                    )}
                  </div>
                </div>
          ...
          <div className="space-y-6">
            {profile.organizationalExperience?.length ? (
              profile.organizationalExperience.map((org, idx) => (
                <div key={idx} className="group flex gap-6 p-8 rounded-3xl border bg-card hover:border-orange-500/50 transition-all duration-300 shadow-sm hover:shadow-xl">
              ...
              <EmptyState icon={Building} text="Belum ada pengalaman organisasi" />
            )}
          </div>
        </div>
      );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
             <div className="space-y-6">
                <div className="flex items-center gap-3 mb-2">
                    <Badge variant="secondary" className="px-2 py-0 text-[10px] uppercase font-black">Utama</Badge>
                ...
                <div className="space-y-4">
                  <HeavyDocumentCard 
                    ...
                    showAnalysis
                  />
                  <HeavyDocumentCard label="Ijazah / SKL" url={profile.ijazahUrl} verified={application.ijazahVerified} />
                  <HeavyDocumentCard label="Portfolio / Website" url={profile.websiteUrl} isUrl />
                </div>
             </div>
             <div className="space-y-6">
                <div className="flex items-center gap-3 mb-2">
                    <Badge variant="secondary" className="px-2 py-0 text-[10px] uppercase font-black">Keahlian</Badge>
                    <h4 className="text-xl font-black">Sertifikasi & Lisensi</h4>
                </div>
                <div className="grid gap-4">
                  {profile.certifications?.length ? (
                    profile.certifications.map((cert, idx) => (
                      <div key={idx} className="group p-5 rounded-3xl border bg-card hover:border-primary transition-all flex items-center justify-between shadow-sm hover:shadow-md">
                         <div className="space-y-1">
                            <p className="font-black text-lg leading-tight group-hover:text-primary transition-colors">{cert.name}</p>
                            <p className="text-sm font-bold text-muted-foreground/80">{cert.organization}</p>
                            <div className="flex items-center gap-2 text-[11px] font-bold text-muted-foreground/60 uppercase tracking-tighter mt-1">
                                <Clock className="h-3 w-3" />
                                {cert.issueDate} {cert.expirationDate ? `— ${cert.expirationDate}` : '(No Expiry)'}
                            </div>
                         </div>
                         {cert.imageUrl && (
                           <Button variant="ghost" size="icon" className="h-12 w-12 rounded-full hover:bg-primary/10 hover:text-primary" asChild>
                    ...
                    <div className="text-center py-10 bg-muted/10 rounded-3xl border border-dashed border-muted-foreground/30">
                      ...
                      <p className="text-muted-foreground text-sm font-medium italic">Belum mengunggah sertifikasi</p>
                    </div>
                  )}
                </div>
             </div>
      ...
      return (
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-12">
              <InfoBlock label="Ekspektasi Gaji" value={profile.salaryExpectation} icon={Clock} />
              <InfoBlock label="Ketersediaan Kerja" value={profile.availability === 'Lainnya' ? profile.availabilityOther : profile.availability} icon={Clock} />
              
              <div className="md:col-span-2 p-8 rounded-3xl bg-primary/5 border border-primary/10">
                 <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground mb-3">Justifikasi Gaji</p>
                 <p className="text-lg font-bold leading-relaxed">{profile.salaryExpectationReason || '-'}</p>
              </div>

              <div className="md:col-span-2 space-y-4">
                 <h4 className="text-lg font-black uppercase tracking-wider text-primary">Motivasi Bergabung</h4>
              ...
              <InfoBlock label="Area Pengembangan" value={profile.improvementArea} />
...
function EmptyState({ icon: Icon, text }: { icon: any, text: string }) {
    return (
        <div className="flex flex-col items-center justify-center py-20 bg-muted/5 rounded-[3rem] border-2 border-dashed border-muted-foreground/20">
            ...
            <p className="text-xl font-bold text-muted-foreground/40 italic">{text}</p>
        </div>
    );
}
  ...
  if (!url) {
    return (
      <div className="flex flex-col gap-2 p-6 rounded-3xl border-2 border-dashed bg-muted/10 items-center justify-center text-center opacity-60">
        ...
        <p className="text-sm font-bold text-muted-foreground/50">{label} tidak tersedia</p>
      </div>
    );
  }
    ...
    <div className="group relative overflow-hidden flex items-center justify-between p-6 rounded-3xl border bg-card hover:shadow-2xl hover:border-primary/50 transition-all duration-500">
      <div className="flex items-center gap-5">
        <div className="h-14 w-14 rounded-2xl bg-primary/10 text-primary flex items-center justify-center group-hover:bg-primary group-hover:text-primary-foreground transition-all duration-500">
           {isUrl ? <Globe className="h-7 w-7" /> : <FileText className="h-7 w-7" />}
        </div>
        <div>
        ...
        {showAnalysis && job && application && profile && (
            <Dialog>
                <DialogTrigger asChild>
                    <Button variant="outline" size="icon" className="rounded-2xl h-14 w-14 shrink-0 shadow-lg border-primary/30 text-primary hover:bg-primary hover:text-primary-foreground transition-all duration-500">
                ...
                <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto rounded-[3rem] p-0 border-none shadow-2xl">
                    <div className="p-8 sm:p-12">
                         <div className="flex items-center gap-4 mb-8">
                            ...
                            <div>
                                <DialogTitle className="text-3xl font-black tracking-tighter">AI Recruitment Analysis</DialogTitle>
                                <p className="text-muted-foreground font-medium">Laporan analisis kecocokan kandidat dengan syarat jabatan.</p>
                            </div>
                         </div>
                         <CandidateFitAnalysis job={job} application={application} profile={profile} compact />
                    </div>
                </DialogContent>
            </Dialog>
        )}
        <Button variant="outline" size="icon" className="rounded-2xl h-14 w-14 shrink-0 shadow-lg group-hover:bg-primary group-hover:border-primary group-hover:text-primary-foreground transition-all duration-500" asChild>
            <a href={url} target="_blank" rel="noopener noreferrer">
            {isUrl ? <ChevronRight className="h-7 w-7" /> : <Eye className="h-7 w-7" />}
            </a>
        </Button>
      </div>
    </div>
...
export function CandidateStepNav({ activeStep, onStepChange }: { activeStep: number; onStepChange: (id: number) => void }) {
    return (
        <div className="flex flex-col gap-1.5">
            ...
            <div className="mt-8 p-4 rounded-2xl bg-muted/30 border border-dashed border-muted-foreground/20 text-center">
                ...
                <p className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-widest">Read Only Mode</p>
            </div>
        </div>
    );
}
...
export function CandidateStepView({ profile, application, job }: { profile: Profile, application: JobApplication, job: Job }) {
    ...
    <Card className="shadow-2xl border-none p-2 rounded-[2.5rem] bg-card/50 backdrop-blur-sm border-t-8 border-t-primary">
       <div className="grid grid-cols-1 md:grid-cols-[220px_1fr] gap-4">
           <div className="p-4 bg-muted/10 rounded-[2rem] border m-2">
                <CandidateStepNav activeStep={activeStep} onStepChange={setActiveStep} />
           </div>
           <div className="p-8 lg:p-12 min-h-[600px]">
                <CandidateStepContent profile={profile} application={application} activeStep={activeStep} job={job} />
           </div>
       </div>
    </Card>
  );
