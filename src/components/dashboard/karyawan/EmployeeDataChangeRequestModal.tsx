...
const formSchema = z.object({
  ...
  requestedData: z.any(),
  supportingDocuments: z.array(z.string()).min(1, "Minimal 1 dokumen pendukung wajib diunggah"),
});
...
export function EmployeeDataChangeRequestModal({ 
  ...
  useEffect(() => {
    const fetchLatestRequest = async () => {
      const cat = initialCategory || form.getValues("category");
      ...
      try {
        ...
        if (!snapshot.empty) {
          setLatestRequest({ id: snapshot.docs[0].id, ...snapshot.docs[0].data() });
        } else {
  ...
  useEffect(() => {
    ...
    if (initialRequestedData) form.setValue("requestedData", initialRequestedData);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of fileList) {
      ...
      const storageRef = ref(storage, `change_requests/${firebaseUser.uid}/${Date.now()}_${file.name}`);
      ...
      uploadTask.on(
        ...
        async () => {
          ...
          form.setValue("supportingDocuments", [...currentDocs, url], { shouldValidate: true });
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      const docRef = await addDoc(collection(firestore, "employee_change_requests"), {
        employeeUid: firebaseUser.uid,
        employeeName: initialProfile.fullName || firebaseUser.displayName || "",
        category: values.category,
        currentData: getCategoryCurrentData(values.category as ChangeRequestCategory, initialProfile),
  ...
  const getCategoryCurrentData = (cat: ChangeRequestCategory, profile: Partial<EmployeeProfile>) => {
    switch (cat) {
      case 'ktp': return { 
      ...
      case 'bpjs_ks': return { 
        ...
        bpjsKesPhotoUrl: profile.dokumenAdministratif?.bpjsKesehatanPhotoUrl || ""
      };
      case 'bpjs_tk': return { 
        bpjsTk: profile.dokumenAdministratif?.bpjsKetenagakerjaan || "",
        bpjsTkPhotoUrl: profile.dokumenAdministratif?.bpjsKetenagakerjaanPhotoUrl || ""
      };
      case 'payroll': return profile.dataRekening || {};
      case 'kk': return {
        noKK: (profile.familyDocuments as any)?.kk?.number || "",
        fileUrl: (profile.familyDocuments as any)?.kk?.fileUrl || ""
      };
      case 'marriage': return {
        noDokumen: (profile.familyDocuments as any)?.marriageCertificate?.number || "",
        fileUrl: (profile.familyDocuments as any)?.marriageCertificate?.fileUrl || ""
      };
      case 'birth_cert': return (profile.familyDocuments as any)?.childBirthCertificates || [];
      case 'spouse_ktp': return {
        nikPasangan: (profile.familyDocuments as any)?.spouseKtp?.nik || "",
        fileUrl: (profile.familyDocuments as any)?.spouseKtp?.fileUrl || ""
      };
      case 'family_bpjs': return (profile.familyDocuments as any)?.familyBpjsMembers || [];
      case 'legal_extra': return (profile.familyDocuments as any)?.additionalDocuments || [];
      default: return {};
    }
  };

  const isPending = latestRequest?.status === "pending";
  const SelectedIcon = CATEGORY_ICONS[form.watch("category") as ChangeRequestCategory] || Info;
      ...
      <DialogContent className="w-[95vw] md:w-[92vw] max-w-[1000px] max-h-[90vh] bg-slate-950 border-slate-800 p-0 overflow-hidden flex flex-col shadow-2xl">
        ...
        <div className="shrink-0 z-10 px-6 py-5 md:px-10 md:py-6 border-b border-slate-800/60 bg-slate-900/90 backdrop-blur-xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm text-slate-400 mt-2 leading-relaxed">
              Perubahan pada data sensitif memerlukan tinjauan HRD. Data Anda akan diperbarui setelah pengajuan ini disetujui.
            </DialogDescription>
          </DialogHeader>
        </div>
          ...
          <div className="p-6 md:p-10">
            {isLoadingLatest ? (
              <div className="flex flex-col items-center justify-center py-20 space-y-4">
                ...
                <p className="text-slate-400 font-medium">Memeriksa status pengajuan...</p>
              </div>
            ) : isPending ? (
              <div className="bg-amber-500/10 border border-amber-500/30 rounded-3xl p-10 flex flex-col items-center text-center space-y-6">
                ...
                <div className="max-w-md">
                  <h4 className="text-xl font-bold text-amber-500 mb-2">Pengajuan Sedang Diproses</h4>
                  <p className="text-slate-400 leading-relaxed">
                    Anda sudah memiliki pengajuan aktif untuk kategori <strong>{CATEGORY_LABELS[latestRequest.category as ChangeRequestCategory]}</strong> yang dikirim pada {new Date(latestRequest.submittedAt?.seconds * 1000).toLocaleDateString('id-ID')}. Mohon tunggu persetujuan HRD sebelum membuat pengajuan baru.
                  </p>
                </div>
                <Button variant="outline" onClick={() => onOpenChange(false)} className="rounded-xl border-slate-800 bg-slate-900/50">
                  Tutup Modal
                </Button>
              </div>
            ) : (
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
                <div className="lg:col-span-3 space-y-8">
                  <Form {...form}>
                    <form id="change-request-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-slate-300 font-bold text-sm tracking-wide">Kategori Perubahan</FormLabel>
                            <Select onValueChange={field.onChange} value={field.value} disabled={!!initialCategory}>
                              <FormControl>
                      ...
                      <div className="bg-slate-900/30 rounded-2xl border border-slate-800/60 p-6 space-y-6">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <h4 className="text-sm font-bold text-white uppercase tracking-wider">Data Baru yang Diajukan</h4>
                        </div>
                        ...
                        {form.watch("category") === 'pajak' && (
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-slate-400 text-xs font-bold uppercase">Nomor NPWP Baru</FormLabel>
                                <FormControl>
                                  <Input 
                                    {...field} 
                        ...
                        {form.watch("category") === 'ktp' && (
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-slate-400 text-xs font-bold uppercase">Nomor NIK Baru</FormLabel>
                                <FormControl>
                                  <Input 
                                    {...field} 
                                    maxLength={16}
                        ...
                        {form.watch("category") === 'bpjs_ks' && (
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-slate-400 text-xs font-bold uppercase">BPJS Kesehatan Baru</FormLabel>
                                <FormControl>
                                  <Input 
                                    {...field} 
                                    className="bg-slate-950/50 border-slate-800 h-12" 
                        ...
                        {form.watch("category") === 'bpjs_tk' && (
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-slate-400 text-xs font-bold uppercase">BPJS Ketenagakerjaan Baru</FormLabel>
                                <FormControl>
                                  <Input 
                                    {...field} 
                                    className="bg-slate-950/50 border-slate-800 h-12" 
                        ...
                        {form.watch("category") === 'payroll' && (
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <FormField
                              ...
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel className="text-slate-400 text-xs font-bold uppercase">Bank Baru</FormLabel>
                                  <FormControl>
                                    <Input {...field} className="bg-slate-950/50 border-slate-800 h-12" placeholder="Contoh: BCA" />
                                  </FormControl>
                                  <FormMessage />
                                </FormItem>
                              )}
                            ...
                            <FormField
                              ...
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel className="text-slate-400 text-xs font-bold uppercase">Nomor Rekening Baru</FormLabel>
                                  <FormControl>
                                    <Input {...field} className="bg-slate-950/50 border-slate-800 h-12" placeholder="000111222" />
                                  </FormControl>
                                  <FormMessage />
                                </FormItem>
                              )}
                            ...
                            <FormField
                              ...
                              render={({ field }) => (
                                <FormItem className="md:col-span-2">
                                  <FormLabel className="text-slate-400 text-xs font-bold uppercase">Nama Pemilik Rekening Baru</FormLabel>
                                  <FormControl>
                                    <Input {...field} className="bg-slate-950/50 border-slate-800 h-12" placeholder="Sesuai buku tabungan" />
                                  </FormControl>
                                  <FormMessage />
                                </FormItem>
                              )}
                        ...
                        {form.watch("category") === 'kk' && (
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-slate-400 text-xs font-bold uppercase">Nomor Kartu Keluarga Baru</FormLabel>
                                <FormControl>
                                  <Input {...field} className="bg-slate-950/50 border-slate-800 h-12" placeholder="16 digit No KK" />
                                </FormControl>
                                <FormMessage />
                              </FormItem>
                            )}
                        ...
                        {form.watch("category") === 'marriage' && (
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-slate-400 text-xs font-bold uppercase">Nomor Buku / Akta Nikah Baru</FormLabel>
                                <FormControl>
                                  <Input {...field} className="bg-slate-950/50 border-slate-800 h-12" placeholder="Sesuai dokumen" />
                                </FormControl>
                                <FormMessage />
                              </FormItem>
                            )}
                        ...
                        {form.watch("category") === 'birth_cert' && (
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-slate-400 text-xs font-bold uppercase">Nama Anak</FormLabel>
                                <Select onValueChange={field.onChange} value={field.value || ""}>
                                  ...
                                  <SelectContent className="bg-slate-900 border-slate-800">
                                    {(initialProfile.familyDocuments as any)?.childBirthCertificates?.map((c: any) => (
                                      <SelectItem key={c.childName} value={c.childName}>{c.childName}</SelectItem>
                                    )) || (
                                      <div className="p-4 text-xs text-slate-500 italic">Belum ada data anak terverifikasi.</div>
                                    )}
                                  </SelectContent>
                                </Select>
                                <FormMessage />
                        ...
                        {form.watch("category") === 'spouse_ktp' && (
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-slate-400 text-xs font-bold uppercase">NIK Pasangan Baru</FormLabel>
                                <FormControl>
                                  <Input {...field} className="bg-slate-950/50 border-slate-800 h-12" placeholder="16 digit NIK Pasangan" />
                                </FormControl>
                                <FormMessage />
                              </FormItem>
                            )}
                        ...
                        {form.watch("category") === 'family_bpjs' && (
                          <div className="space-y-6">
                            <FormField
                              ...
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel className="text-slate-400 text-xs font-bold uppercase">Anggota Tanggungan</FormLabel>
                                  <Select onValueChange={field.onChange} value={field.value || ""}>
                                    <FormControl>
                                      <SelectTrigger className="bg-slate-950/50 border-slate-800 h-12 text-white">
                                    ...
                                    <SelectContent className="bg-slate-900 border-slate-800">
                                      {(initialProfile.familyDocuments as any)?.familyBpjsMembers?.map((m: any) => (
                                        <SelectItem key={m.dependentName} value={m.dependentName}>{m.dependentName} ({m.relationship})</SelectItem>
                                      )) || (
                                        <div className="p-4 text-xs text-slate-500 italic">Belum ada data tanggungan terverifikasi.</div>
                                      )}
                                    </SelectContent>
                                  </Select>
                                  <FormMessage />
                            ...
                            <FormField
                              ...
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel className="text-slate-400 text-xs font-bold uppercase">Nomor BPJS Baru</FormLabel>
                                  <FormControl>
                                    <Input {...field} className="bg-slate-950/50 border-slate-800 h-12" placeholder="No kartu BPJS" />
                                  </FormControl>
                                  <FormMessage />
                                </FormItem>
                              )}
                        ...
                        {form.watch("category") === 'legal_extra' && (
                          <div className="space-y-6">
                            <FormField
                              ...
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel className="text-slate-400 text-xs font-bold uppercase">Jenis Dokumen</FormLabel>
                                  <Select onValueChange={field.onChange} value={field.value || ""}>
                                    <FormControl>
                                      <SelectTrigger className="bg-slate-950/50 border-slate-800 h-12 text-white">
                                    ...
                                    <SelectContent className="bg-slate-900 border-slate-800">
                                      {["Surat Cerai", "Akta Kematian", "Dokumen Wali / Adopsi", "Dokumen Lainnya"].map((t) => (
                            ...
                            />
                            {form.watch("requestedData.documentType") === "Dokumen Lainnya" && (
                              <FormField
                                control={form.control}
                                name="requestedData.documentName"
                                render={({ field }) => (
                                  <FormItem>
                                    <FormLabel className="text-slate-400 text-xs font-bold uppercase">Nama Dokumen</FormLabel>
                                    <FormControl>
                                      <Input {...field} className="bg-slate-950/50 border-slate-800 h-12" placeholder="Tulis nama dokumen" />
                                    </FormControl>
                                    <FormMessage />
                                  </FormItem>
                                )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="space-y-4">
                            <div>
                              <FormLabel className="text-slate-300 font-bold text-sm tracking-wide">Dokumen Pendukung</FormLabel>
                              <FormDescription className="text-xs text-slate-400 mt-1 leading-relaxed">
                                Wajib mengunggah bukti legal (KTP, NPWP, Akta, dll) sesuai kategori yang diajukan.
                              </FormDescription>
                            </div>
                            
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {field.value.map((url, index) => (
                                <div key={index} className="relative group rounded-xl overflow-hidden border border-slate-800 bg-slate-900/50 p-2">
                                  <div className="h-32 w-full rounded-lg bg-slate-950 flex items-center justify-center">
                                    <img src={url} alt="Proof" className="h-full w-full object-contain" />
                                  </div>
                                  <Button 
                                    type="button" 
                                    variant="destructive" 
                                    ...
                                    onClick={() => {
                                      ...
                                      next.splice(index, 1);
                                      form.setValue("supportingDocuments", next);
                                    }}
                              ...
                              <div className="relative h-36 rounded-xl border-2 border-dashed border-slate-800 bg-slate-900/20 hover:bg-slate-900/40 transition-all flex flex-col items-center justify-center p-4 text-center cursor-pointer group">
                                ...
                                <span className="text-xs font-bold text-slate-400">Klik untuk upload</span>
                              </div>
                            </div>
                            ...
                            {Object.entries(uploadProgress).map(([id, progress]) => (
                              <div key={id} className="space-y-1">
                                ...
                                <p className="text-[10px] text-blue-400 font-bold uppercase">Uploading... {progress}%</p>
                              </div>
                            ))}
        ...
        {!isPending && !isLoadingLatest && (
          <div className="shrink-0 z-10 px-6 py-5 md:px-10 md:py-6 border-t border-slate-800/60 bg-slate-900/90 backdrop-blur-xl flex justify-end gap-3">
            ...
            >
              {isSubmitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Memproses...</> : "Kirim Pengajuan"}
            </Button>
          </div>
        )}
      </DialogContent>
