...
const formSchema = z.object({
  ...
  requestedAccountNumber: z
    .string()
    .min(5, "Nomor rekening wajib diisi dan minimal 5 digit")
    ...
    try {
      const docRef = await addDoc(
        collection(firestore, "bank_change_requests"),
        {
          ...
          employeeEmail: firebaseUser.email || "",
          currentBankName: initialProfile.dataRekening?.bankName || "",
          currentAccountNumber:
            initialProfile.dataRekening?.bankAccountNumber || "",
        ...
        <div className="shrink-0 z-10 px-6 py-5 md:px-10 md:py-6 border-b border-slate-800/60 bg-slate-900/90 backdrop-blur-xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm lg:text-base text-slate-400 mt-2 max-w-3xl leading-relaxed">
              Gunakan form ini jika Anda ingin mengganti rekening payroll. Data
              baru tidak langsung aktif dan harus diverifikasi HRD terlebih
              dahulu.
            </DialogDescription>
          </DialogHeader>
        </div>
          ...
          <div className="p-6 md:p-10">
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-14">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="space-y-4">
                          <div>
                            <FormLabel className="text-slate-300 font-bold text-sm tracking-wide">
                              Bukti Rekening / Buku Tabungan Baru
                            </FormLabel>
                            <FormDescription className="text-sm text-slate-400 mt-1.5 leading-relaxed">
                              Upload foto buku tabungan, screenshot m-banking,
                              atau dokumen bank yang menampilkan nama dan nomor
                              rekening.
                            </FormDescription>
                          </div>
                          <FormControl>
                            <div className="mt-4">
                              {field.value ? (
                                <div className="relative group w-full rounded-xl overflow-hidden border border-slate-700 bg-slate-900/50 p-3">
                                  ...
                                  <div className="absolute inset-0 bg-slate-950/70 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center backdrop-blur-[2px]">
                                    <Button
                                      ...
                                      variant="outline"
                                      onClick={() =>
                                        form.setValue("requestedProofUrl", "")
                                      }
                                      className="rounded-xl border-red-500/50 bg-red-500/10 hover:bg-red-500/20 text-red-400 h-12 px-6"
                                    >
                                      <XCircle className="w-5 h-5 mr-2" /> Hapus
                                      & Ganti File
                                    </Button>
                                  </div>
                                </div>
                                ...
                                <div className="relative w-full h-56 rounded-xl border-2 border-dashed border-slate-700/80 bg-slate-900/20 hover:bg-slate-900/40 hover:border-slate-600 transition-all flex flex-col items-center justify-center p-8 text-center cursor-pointer group">
                                  ...
                                  <div className="text-base font-bold text-slate-300 mb-1.5">
                                    Klik atau Drag & Drop dokumen ke sini
                                  </div>
                                  <div className="text-sm text-slate-500 font-medium">
                                    Max. ukuran file: 5MB (JPG, PNG, PDF)
                                  </div>
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <div
                  className={`rounded-2xl border p-6 lg:p-7 shadow-lg ${
                    !latestRequest
                  ...
                  {!latestRequest && (
                    <div>
                      ...
                      <p className="text-sm text-slate-400">
                        Anda belum memiliki pengajuan perubahan rekening aktif.
                      </p>
                    </div>
                  )}
                  {latestRequest?.status === "pending" && (
                    <div>
                      ...
                      <p className="text-sm text-amber-500/80">
                        Pengajuan Anda sedang diperiksa oleh HRD.
                      </p>
                    </div>
                  )}
                  {latestRequest?.status === "approved" && (
                    <div>
                      ...
                      <p className="text-sm text-emerald-500/80">
                        Data rekening terbaru sudah menjadi data aktif payroll.
                      </p>
                    </div>
                  )}
                  {latestRequest?.status === "rejected" && (
                    <div>
                      ...
                      <p className="text-sm text-red-400">
                        Pengajuan belum dapat disetujui. Silakan lihat catatan
                        HRD.
                      </p>
                ...
                <div className="bg-slate-900/40 rounded-2xl border border-slate-800/60 p-7 lg:p-8">
                  ...
                  <div className="space-y-5">
                    <div className="flex gap-4">
                      <div className="shrink-0 w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-xs font-bold text-slate-300">
                        1
                      </div>
                      <div>
                        ...
                        <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                          Karyawan mengirim data rekening baru.
                        </p>
                      </div>
                    </div>
                    <div className="flex gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                          HRD memeriksa data dan bukti rekening.
                        </p>
                      </div>
                    </div>
                    <div className="flex gap-4">
                      <div className="shrink-0 w-8 h-8 rounded-full bg-emerald-900/30 text-emerald-500 border border-emerald-500/30 flex items-center justify-center text-xs font-bold">
                        3
                      </div>
                      <div>
                        <p className="text-sm font-bold text-slate-200">
                        ...
                        <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                          Jika disetujui, rekening payroll diperbarui. Jika
                          ditolak, rekening lama tetap digunakan.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-amber-500/5 rounded-2xl border border-amber-500/20 p-6 flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-500/90 leading-relaxed font-medium">
                      Selama pengajuan belum disetujui HRD, payroll tetap
                      menggunakan rekening aktif sebelumnya.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="shrink-0 z-10 px-6 py-5 md:px-10 md:py-6 border-t border-slate-800/60 bg-slate-900/90 backdrop-blur-xl flex flex-col md:flex-row md:items-center justify-between gap-4 mt-auto">
          <div>
            {isPending && (
              <p className="text-sm text-amber-500 font-medium">
                Anda masih memiliki pengajuan rekening yang menunggu persetujuan
                HRD.
              </p>
            )}
          </div>
          <div className="flex flex-col-reverse md:flex-row gap-3 w-full md:w-auto">
            ...
            >
              {isSubmitting ? (
                <>
                  <Loader2 className="mr-3 h-5 w-5 animate-spin" /> Sedang
                  Memproses...
                </>
              ) : (
                "Kirim Pengajuan"
