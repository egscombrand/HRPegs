...
const AddressView = ({
  ...
  return (
    <div>
      <p className="text-xs font-medium text-muted-foreground">{title}</p>
      <p className="text-base font-semibold mt-0.5 whitespace-pre-line">
        {fullAddress}
      </p>
    </div>
  );
...
export function ProfilePreview({
  ...
  const birthDateValue = React.useMemo(() => {
    ...
    if (!date) return profile.birthDate ? "Invalid Date" : "-";
    return format(date, "dd MMMM yyyy", { locale: idLocale });
  }, [profile.birthDate]);

  const updatedAtValue = React.useMemo(() => {
    if (!profile.updatedAt) return null;
    const date = (profile.updatedAt as any)?.toDate
      ? (profile.updatedAt as any).toDate()
      : new Date(profile.updatedAt as any);
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <CardDescription>
                ...
                <span className="text-xs">
                  Terakhir diperbarui: {updatedAtValue || "Baru saja"}
                </span>
              </CardDescription>
            </div>
            <div className="flex items-center gap-4">
              ...
              <Button
                ...
                disabled={isProfileLocked || isLoadingApps}
              >
                {isLoadingApps ? (
      ...
      <div className="grid lg:grid-cols-3 gap-6 items-start">
          ...
          <Card>
            ...
            <CardContent>
              <dl className="grid md:grid-cols-2 gap-x-6 gap-y-6">
                ...
                <InfoRow
                  label="Bersedia WFO"
                  value={profile.willingToWfo ? "Ya" : "Tidak"}
                />
                {profile.linkedinUrl && (
                  <InfoRow
                    label="LinkedIn"
                    value={
                      ...
                      >
                        <LinkIcon className="h-3 w-3" /> Profil LinkedIn
                      </a>
                    }
                  />
                )}
                  ...
                  <InfoRow
                    ...
                    value={
                      <a
                        ...
                        className="text-primary hover:underline flex items-center gap-1"
                      >
                        <LinkIcon className="h-3 w-3" /> Link Portofolio
                      </a>
                    }
                  />
                )}
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {profile.isDomicileSameAsKtp ? (
                ...
                <AddressView
                  ...
                  address={profile.addressDomicile}
                />
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  <InfoRow
                    label="Bekerja dengan Target/Deadline"
        ...
        <div className="lg:sticky lg:top-24 space-y-6">
            ...
            <CardContent className="space-y-4">
              {profile.education?.length > 0 ? (
                ...
                <p className="text-sm text-muted-foreground p-4 text-center">
                  Belum ada riwayat pendidikan. Tambahkan untuk meningkatkan
                  peluang Anda.
                </p>
              )}
            </CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {profile.workExperience && profile.workExperience.length > 0 ? (
                profile.workExperience.map((item, i) => (
                  <WorkExperienceView key={i} item={item} />
                ))
              ) : (
                <p className="text-sm text-muted-foreground p-4 text-center">
                  Belum ada pengalaman kerja. Tambahkan untuk meningkatkan
                  peluang Anda.
                </p>
              )}
            </CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {profile.organizationalExperience &&
              profile.organizationalExperience.length > 0 ? (
                profile.organizationalExperience.map((item, i) => (
                  <OrgExperienceView key={i} item={item} />
                ))
              ) : (
                <p className="text-sm text-muted-foreground p-4 text-center">
