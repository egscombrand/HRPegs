...
export default function JobApplyPage() {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const userApplications = userAppsSnap.docs.map(
        (doc) => ({ id: doc.id, ...doc.data() }) as JobApplication,
      );
      ...
      if (finalApps.length > 0) {
        ...
        const decisionDate =
          lastFinalApp.decisionAt?.toDate() || lastFinalApp.updatedAt.toDate();
      ...
      if (
        !profileSnap.exists() ||
        !profileSnap.data().cvUrl ||
        !profileSnap.data().ijazahUrl
      ) {
        toast({
          variant: "destructive",
          title: "Dokumen Belum Lengkap",
          description:
            "Anda harus melengkapi profil dan mengunggah CV serta Ijazah sebelum melamar.",
        });
      ...
      await setDocumentNonBlocking(applicationRef, applicationData, {
        merge: false,
      });
        ...
        <Card>
          <CardHeader>
            <div>
              ...
              <CardDescription>
                Profil Anda akan dikirimkan sebagai lamaran untuk posisi ini.
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-center p-8 border rounded-lg bg-muted/50">
              Profil Anda akan dikirimkan sebagai lamaran untuk posisi ini.
              Pastikan semua data di profil Anda sudah benar sebelum mengirim.
            </p>
