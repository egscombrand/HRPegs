  ...
  return (
    <Badge 
      ...
      className={cn(
        ...
        variant === "success" ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" :
...
export default function KaryawanDataPage() {
  ...
  const allMerged = useMemo<MergedEmployee[]>(() => {
    ...
    const systemRoles = ["super-admin", "system-admin", "admin-system", "super_admin", "system_admin", "admin_system"];
    ...
    (employees ?? []).forEach((emp) => {
      ...
      result.push({
        uid: emp.uid,
        fullName: emp.fullName || profile?.fullName || profile?.dataDiriIdentitas?.fullName || "",
        email: emp.email || profile?.email || "",
        employeeNumber: (emp.employeeNumber || profile?.employeeNumber) ?? undefined,
        positionTitle: normalized.jabatan,
        division: normalized.divisi,
        brandId: normalized.brandId,
        brandName: normalized.brandName,
        employmentStatus: emp.employmentStatus || profile?.hrdEmploymentInfo?.statusKerja || "",
  ...
  const filteredEmployees = useMemo(() => {
    return allMerged.filter((emp) => {
      ...
      else if (activeTab === "inactive") tabMatch = status === "resigned" || status === "terminated";

      if (!tabMatch) return false;
      const brandMatch = brandFilter === "all" || emp.brandId === brandFilter;
      if (!brandMatch) return false;
      const searchMatch = searchTerm === "" || emp.fullName.toLowerCase().includes(searchTerm.toLowerCase()) || emp.email.toLowerCase().includes(searchTerm.toLowerCase());
      if (!searchMatch) return false;
      if (completenessFilter !== "all") {
        const c = calculateProfileCompleteness(emp.employeeProfile ?? null);
        if (completenessFilter === "complete" && c.status !== "complete") return false;
        if (completenessFilter === "partial" && c.status !== "partial") return false;
        if (completenessFilter === "not_started" && c.status !== "not_started") return false;
      }
      return true;
    });
  ...
  const groupedEmployees = useMemo(() => {
    ...
    filteredEmployees.forEach((emp) => {
      if (groups[emp.operationalStatus]) groups[emp.operationalStatus].push(emp);
  ...
  const stats = useMemo(() => {
    return {
      ...
      training: allMerged.filter(e => e.operationalStatus === 'training').length,
      intern: allMerged.filter(e => e.operationalStatus === 'intern').length,
      probation: allMerged.filter(e => e.operationalStatus === 'probation').length,
      contract: allMerged.filter(e => e.operationalStatus === 'contract').length,
      inactive: allMerged.filter(e => ['resigned', 'terminated'].includes(e.operationalStatus)).length,
    };
  }, [allMerged]);
  ...
  const handleExport = () => {
    ...
    const headers = ["fullName", "email", "positionTitle", "division", "brandName", "employmentStatus", "employeeNumber"];
    const csvContent = [headers.join(","), ...filteredEmployees.map((e) => headers.map((h) => `"${((e as any)[h] || "").toString().replace(/"/g, '""')}"`).join(",")),].join("\n");
        ...
        <div className="space-y-8 w-full max-w-[1800px] mx-auto px-4 md:px-8 pb-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" className="rounded-xl border-slate-800 bg-slate-900/50 text-slate-400 hover:text-white" onClick={() => setIsImportOpen(true)}>
                <Upload className="mr-2 h-3.5 w-3.5" /> Import
              </Button>
              <Button variant="outline" size="sm" className="rounded-xl border-slate-800 bg-slate-900/50 text-slate-400 hover:text-white" onClick={handleExport}>
                <Download className="mr-2 h-3.5 w-3.5" /> Export
              </Button>
              <Button variant="outline" size="sm" className="rounded-xl border-slate-800 bg-slate-900/50 text-slate-400 hover:text-white" onClick={handleDownloadTemplate}>
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            ...
            <SummaryCard label="Aktif" count={stats.active} icon={CheckCircle} color="bg-emerald-500/10 text-emerald-500" />
            <SummaryCard label="Training" count={stats.training} icon={GraduationCap} color="bg-amber-500/10 text-amber-500" />
            <SummaryCard label="Magang" count={stats.intern} icon={Briefcase} color="bg-indigo-500/10 text-indigo-500" />
            <SummaryCard label="Percobaan" count={stats.probation} icon={AlertTriangle} color="bg-orange-500/10 text-orange-500" />
            <SummaryCard label="Kontrak" count={stats.contract} icon={FileSpreadsheet} color="bg-blue-500/10 text-blue-500" />
            <SummaryCard label="Nonaktif" count={stats.inactive} icon={XCircle} color="bg-red-500/10 text-red-500" />
          </div>
          ...
          <Card className="border-slate-800 bg-slate-950/50 backdrop-blur-xl">
             <CardHeader className="p-5 border-b border-slate-800/50">
                <div className="flex flex-wrap items-center gap-4">
                   ...
                   {(searchTerm || brandFilter !== "all" || completenessFilter !== "all" || activeTab !== "all") && (
             ...
             <CardContent className="p-0">
               {isLoading ? (
                  <div className="p-20 text-center space-y-4">
                     ...
                     <p className="text-sm text-slate-500 font-medium">Menyusun direktori karyawan...</p>
                  </div>
                  ...
                  <Accordion type="multiple" defaultValue={Object.keys(groupedEmployees)} className="w-full">
                     {Object.entries(groupedEmployees)
                        ...
                        .map(([status, emps]) => (
                        <AccordionItem value={status} key={status} className="border-b border-slate-800/50 last:border-0">
                           <AccordionTrigger className="px-6 py-3 bg-slate-900/20 hover:no-underline group">
                              <div className="flex items-center gap-3">
                                 <StatusKepegawaanBadge status={status as OperationalStatus} />
                                 <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">{emps.length} Personel</span>
                              </div>
                           </AccordionTrigger>
                  ...
                  <div className="p-20 text-center space-y-4">
                     ...
                     <p className="text-sm text-slate-500 font-medium">Tidak ada data yang sesuai dengan filter saat ini.</p>
                     <Button variant="outline" size="sm" className="rounded-xl border-slate-800" onClick={() => {
                        setSearchTerm("");
                        setBrandFilter("all");
                        setCompletenessFilter("all");
                        setActiveTab("all");
