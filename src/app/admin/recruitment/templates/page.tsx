...
export default function OfferingTemplatesPage() {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<OfferingTemplate | null>(null);
  ...
  const templatesQuery = useMemoFirebase(
    () => query(collection(firestore, "recruitment_offering_templates"), orderBy("updatedAt", "desc")),
    [firestore]
  );
  const { data: templates, isLoading, error } = useCollection<OfferingTemplate>(templatesQuery);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(firestore, "recruitment_offering_templates", templateToDelete));
      toast({ title: "Template Dihapus", description: "Template telah berhasil dihapus dari sistem." });
    } catch (err: any) {
  ...
  return (
    <DashboardLayout pageTitle="Master Template Offering" menuConfig={menuConfig}>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Master Template Offering</h2>
          <p className="text-muted-foreground">Kelola template resmi perusahaan untuk generate PDF penawaran kerja.</p>
        </div>
                  ...
                  <TableRow key={template.id}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive" onClick={() => confirmDelete(template.id!)}>
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tindakan ini tidak dapat dibatalkan. Template yang dihapus tidak akan tersedia lagi untuk pembuatan offering baru.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
