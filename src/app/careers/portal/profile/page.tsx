...
function ProfileWizardContent() {
  ...
  useEffect(() => {
    const createMissingProfile = async () => {
      if (
        ...
        console.warn(
          "Profile document not found for this user, creating a new one as a fallback.",
        );
        ...
        try {
          await setDocumentNonBlocking(profileDocRef, defaultProfileData, {
            merge: true,
          });
          toast({
            title: "Profil Dibuat",
            description: "Memulai profil baru untuk Anda.",
          });
          refreshProfile();
        } catch (e: any) {
      ...
      <Card className="flex flex-col items-center justify-center text-center p-8">
        <CardHeader>
          <CardTitle className="mt-4 text-2xl">
            Profil Pelamar Belum Dibuat
          </CardTitle>
        </CardHeader>
        <CardContent className="w-full max-w-sm">
          <p className="text-muted-foreground mb-6">
            Anda belum memiliki profil pelamar. Mulai isi profil Anda untuk
            dapat melamar pekerjaan.
          </p>
  ...
  if (isPreviewMode) {
    return (
      <ProfilePreview profile={profile} onEditRequest={handleEnterEditMode} />
    );
  }
      ...
      {effectiveStep === 5 && (
        <SkillsForm
          initialData={{
            ...
            cvUrl: initialProfileData.cvFileId
              ? {
                  url: initialProfileData.cvUrl || "",
                  fileId: initialProfileData.cvFileId,
                  fileName: initialProfileData.cvFileName,
                  googleDriveWebViewLink:
                    initialProfileData.cvGoogleDriveWebViewLink,
                }
              : initialProfileData.cvUrl || "",
            ijazahUrl: initialProfileData.ijazahFileId
              ? {
                  url: initialProfileData.ijazahUrl || "",
                  fileId: initialProfileData.ijazahFileId,
                  fileName: initialProfileData.ijazahFileName,
                  googleDriveWebViewLink:
                    initialProfileData.ijazahGoogleDriveWebViewLink,
                }
              : initialProfileData.ijazahUrl || "",
          }}
          onSaveSuccess={handleSaveSuccess}
          onBack={handleBack}
        />
