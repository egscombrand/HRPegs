...
function SubmittedDocumentsView({
  application,
}: {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Dokumen Anda untuk lamaran ini telah berhasil dikirim dan sedang dalam
          proses verifikasi oleh tim HRD.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <Alert variant="default">
          ...
          <AlertDescription>
            Anda dapat memperbarui dokumen jika diperlukan. Tim kami akan
            menghubungi Anda jika ada informasi lebih lanjut.
          </AlertDescription>
        </Alert>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
          ...
          {application.ijazahUrl && application.ijazahFileName && (
            <div className="p-4 border rounded-lg">
              ...
              <p
                className="text-muted-foreground text-sm truncate"
                title={application.ijazahFileName}
              >
...
export default function DocumentsPage() {
  ...
  const applicationsForEditing = useMemo(
    () =>
      applications?.filter((app) =>
        ["document_submission", "interview"].includes(app.status),
      ) || [],
    [applications],
  );
      ...
      setUploads({
        cv:
          app.cvUrl && app.cvFileName
            ? { url: app.cvUrl, name: app.cvFileName }
            : null,
        ijazah:
          app.ijazahUrl && app.ijazahFileName
            ? { url: app.ijazahUrl, name: app.ijazahFileName }
            : null,
      });
    }
  ...
  const handleSubmitDocuments = async () => {
    ...
    const isFirstSubmissionForAny = applicationsForEditing.some(
      (app) => app.status === "document_submission",
    );
    ...
    try {
      await batch.commit();
      toast({
        title: isFirstSubmissionForAny
          ? "Dokumen Berhasil Dikirim"
          : "Dokumen Diperbarui",
        description: isFirstSubmissionForAny
          ? "Selamat, Anda lolos ke tahap wawancara!"
          : "Perubahan dokumen Anda telah disimpan.",
      });
  ...
  const isFirstSubmission = applicationsForEditing.some(
    (app) => app.status === "document_submission",
  );
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Kelola CV, portofolio, dan dokumen pendukung lainnya.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="text-center p-8 border rounded-lg bg-muted/50 flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-muted-foreground max-w-md">
              Saat ini tidak ada lamaran Anda yang memerlukan pengumpulan
              dokumen. Anda akan melihat formulir unggah di sini jika sudah
              waktunya.
            </p>
          </div>
        </CardContent>
      ...
      {applicationsForEditing.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Unggah dokumen yang diminta. File yang diunggah akan menggantikan
              versi sebelumnya.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <Alert>
              ...
              <AlertDescription>
                Dokumen yang Anda unggah di sini akan digunakan untuk semua
                lamaran yang sedang dalam tahap ini (
                {applicationsForEditing.length} lamaran).
              </AlertDescription>
            </Alert>
