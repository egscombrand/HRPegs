...
function StartTestForApplication({ applicationId }: { applicationId: string }) {
  ...
  const {
    ...
    error: appError,
  } = useDoc<JobApplication>(appRef);

  const assessmentRef = useMemoFirebase(
    () => doc(firestore, "assessments", "default"),
    [firestore],
  );
  const { data: activeAssessment, isLoading: assessmentLoading } =
    useDoc<Assessment>(assessmentRef);

  const configDocRef = useMemoFirebase(
    () => doc(firestore, "assessment_config", "main"),
    [firestore],
  );
  const { data: assessmentConfig, isLoading: configLoading } =
    useDoc<AssessmentConfig>(configDocRef);
  ...
  useEffect(() => {
    ...
    if (
      ...
      activeAssessment.publishStatus !== "published"
    ) {
      if (appError) {
        toast({
          variant: "destructive",
          title: "Error",
          description: `Gagal memuat detail lamaran: ${appError.message}`,
        });
      } else {
        toast({
          variant: "destructive",
          title: "Error",
          description:
            "Gagal mempersiapkan tes. Lamaran, tes, atau konfigurasi tidak valid.",
        });
      }
    ...
    if (application.candidateUid !== userProfile.uid) {
      toast({
        variant: "destructive",
        title: "Akses Ditolak",
        description: "Anda tidak diizinkan untuk memulai sesi tes ini.",
      });
      router.push("/careers/portal/applications");
      return;
    }
    ...
    const handleStart = async () => {
      const deadline = application.personalityTestAssignedAt
        ? new Date(
            application.personalityTestAssignedAt.toDate().getTime() +
              24 * 60 * 60 * 1000,
          )
        : null;
      ...
      const existingSessionData =
        existingSessionDoc?.data() as AssessmentSession;

      if (isExpired && existingSessionData?.status !== "submitted") {
        toast({
          variant: "destructive",
          title: "Waktu Habis",
          description: "Waktu pengerjaan tes untuk lamaran ini telah berakhir.",
        });
        router.push("/careers/portal/applications");
        return;
      }
      ...
      const likertQuery = query(
        questionsCollection,
        where("assessmentId", "==", "default"),
        where("isActive", "==", true),
        where("type", "==", "likert"),
      );
      const forcedChoiceQuery = query(
        questionsCollection,
        where("assessmentId", "==", "default"),
        where("isActive", "==", true),
      ...
      const [likertQuestionsSnap, forcedChoiceQuestionsSnap] =
        await Promise.all([getDocs(likertQuery), getDocs(forcedChoiceQuery)]);

      const likertIds = likertQuestionsSnap.docs.map((d) => d.id);
      const forcedChoiceIds = forcedChoiceQuestionsSnap.docs.map(
        (doc) => doc.id,
      );

      const likertCount =
        (assessmentConfig?.bigfiveCount || 30) +
        (assessmentConfig?.discCount || 20);
      const forcedChoiceCount = assessmentConfig?.forcedChoiceCount || 20;

      if (
        likertIds.length < likertCount ||
        forcedChoiceIds.length < forcedChoiceCount
      ) {
      ...
      if (existingSessionDoc) {
        const isOldSession =
          !existingSessionData.selectedQuestionIds?.forcedChoice ||
          existingSessionData.selectedQuestionIds.forcedChoice.length === 0;

        if (isOldSession || isRetry) {
          toast({
            title: "Sesi Tes Diperbarui",
            description:
              "Sesi lama Anda tidak valid. Membuat ulang sesi tes untuk Anda.",
          });
          ...
          router.push(
            `/careers/portal/assessment/personality/${existingSessionDoc.id}`,
          );
          return;
        }
        ...
        if (existingSessionData.status === "submitted") {
          toast({
            title: "Tes Selesai",
            description:
              "Anda sudah menyelesaikan tes untuk lowongan ini. Melihat hasil...",
          });
          router.push(
            `/careers/portal/assessment/personality/result/${existingSessionDoc.id}`,
          );
        } else {
          // If session is valid and in draft
          toast({
            title: "Melanjutkan Sesi",
            description: "Anda akan melanjutkan tes yang sedang berjalan.",
          });
          router.push(
            `/careers/portal/assessment/personality/${existingSessionDoc.id}`,
          );
      ...
      if (deadline != null) {
        sessionData.deadlineAt = Timestamp.fromDate(deadline);
      }

      console.debug("[Assessment Start] Creating assessment_sessions payload", {
        applicationId,
        candidateUid: userProfile.uid,
        deadlineAt: sessionData.deadlineAt ?? null,
  ...
  const handleStartTest = async () => {
    if (!userProfile) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Anda harus login untuk memulai tes.",
      });
      return;
    }
    setIsStarting(true);
    try {
      ...
      if (!existingSessionsSnap.empty) {
        ...
        if (
          session.deadlineAt &&
          new Date() > session.deadlineAt.toDate() &&
          ...
          toast({
            ...
            title: "Waktu Habis",
            description:
              "Waktu pengerjaan tes untuk lamaran ini telah berakhir.",
          });
          router.push("/careers/portal/applications");
          setIsStarting(false);
          return;
        ...
        toast({
          title: "Sesi Ditemukan",
          description: "Anda akan diarahkan ke sesi tes yang sudah ada.",
        });
        if (session.status === "submitted") {
          router.push(
            `/careers/portal/assessment/personality/result/${sessionDoc.id}`,
          );
        } else {
          router.push(
            `/careers/portal/assessment/personality/${sessionDoc.id}`,
          );
        }
        return;
      }
  ...
  return (
    <Card className="max-w-3xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>
          Tes ini dirancang untuk membantu kami memahami preferensi dan gaya
          kerja Anda. Hasil tes ini akan menjadi patokan umum untuk semua
          lamaran Anda.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <Alert>
          ...
          <AlertDescription>
            Anda hanya perlu mengerjakan tes ini satu kali. Hasilnya akan kami
            simpan dan gunakan sebagai referensi untuk lamaran-lamaran Anda
            selanjutnya. Jika Anda sudah pernah mengerjakan, kami akan
            melanjutkan sesi Anda atau menampilkan hasilnya.
          </AlertDescription>
        </Alert>
...
function AssessmentStartPageContent() {
  ...
  const { data: activeTestApplications, isLoading: activeTestAppsLoading } =
    useCollection<JobApplication>(activeTestApplicationQuery);
  ...
  if (!isLoading && activeTestApplication) {
    return (
      <StartTestForApplication applicationId={activeTestApplication.id!} />
    );
  }
